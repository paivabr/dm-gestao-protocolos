...
export async function getChecklistTemplates() {
  ...
  try {
    const { checklistTemplates, checklistTemplateItems } = await import("../drizzle/schema");
...
export async function getChecklistTemplateById(id: number) {
  ...
  try {
    ...
    const items = await db.select().from(checklistTemplateItems).where(eq(checklistTemplateItems.templateId, id));
    
    return {
      ...template[0],
      itens: items.map(item => item.descricao),
    };
  } catch (error) {
...
export async function createChecklistTemplate(nome: string, descricao: string | undefined, itens: string[], userId: number) {
  const db = await getDb();
  if (!db) {
  ...
  try {
    ...
    return { id: templateId, nome, descricao, itens };
  } catch (error) {
