export function gerarCSVProtocolos(protocolos: any[]): string {
  ...
  const headers = [
    ...
    'Data Pagamento Despesas',
    'Total Despesas Pendentes',
    'Total Receitas',
    'Total Recebido',
    'Data Pagamento Receitas',
    'Total Pendente',
    'Custas',
  ...
  const rows = protocolos.map(p => [
    ...
    p.dataPagamentoDespesas ? new Date(p.dataPagamentoDespesas).toLocaleDateString('pt-BR') : '',
    (Number(p.totalDespesasPendentes) || 0).toFixed(2),
    (Number(p.totalReceitas) || 0).toFixed(2),
    (Number(p.totalRecebido) || 0).toFixed(2),
    p.dataPagamentoReceitas ? new Date(p.dataPagamentoReceitas).toLocaleDateString('pt-BR') : '',
    (Number(p.totalPendente) || 0).toFixed(2),
    (Number(p.custas) || 0).toFixed(2),
    p.dataPagamentoCustas ? new Date(p.dataPagamentoCustas).toLocaleDateString('pt-BR') : '',
